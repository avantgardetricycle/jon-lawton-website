...
`index.html` is the homepage (~4MB because images and videos are inlined as base64). `about.html` is the long-form About page. `contact.html` is booking and press contact. Edit surgically. Do not rewrite whole files.
...
1. Sticky `.controls-bar` — logo plus nav: Music (`#release` / `index.html#release`), Shows (`#shows` / `index.html#shows`), About (`about.html`), Contact (`contact.html`)
